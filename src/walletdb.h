...
// Copyright (c) 2016-2018 The Profitbase developers
...
public:
    ...
    bool WriteZPBCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPBCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPBCSeed_deprecated(uint256& seed);
    bool EraseZPBCSeed();
    bool EraseZPBCSeed_deprecated();

    bool WriteZPBCCount(const uint32_t& nCount);
    bool ReadZPBCCount(uint32_t& nCount);
