...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZPbcStake(spend));
    } else {
        ...
        CPbcStake* pbcInput = new CPbcStake();
        pbcInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(pbcInput);
    }
