...
#include "ui_zpbccontroldialog.h"
...
std::set<std::string> ZPbcControlDialog::setSelectedMints;
std::set<CMintMeta> ZPbcControlDialog::setMints;

ZPbcControlDialog::ZPbcControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZPbcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZPbcControlDialog::~ZPbcControlDialog()
{
    delete ui;
}

void ZPbcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPbcControlDialog::updateList()
{
...
void ZPbcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPbcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZPbc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPbcControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZPbcControlDialog::GetSelectedMints()
{
...
void ZPbcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
