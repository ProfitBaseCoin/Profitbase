...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::PBC);
    ...
    if (!settings.contains("nAnonymizeProfitbaseAmount"))
        settings.setValue("nAnonymizeProfitbaseAmount", 1000);

    nAnonymizeProfitbaseAmount = settings.value("nAnonymizeProfitbaseAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeProfitbaseAmount"))
        SoftSetArg("-anonymizeprofitbaseamount", settings.value("nAnonymizeProfitbaseAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in profitbase.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeProfitbaseAmount:
            return QVariant(nAnonymizeProfitbaseAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeProfitbaseAmount:
            nAnonymizeProfitbaseAmount = value.toInt();
            settings.setValue("nAnonymizeProfitbaseAmount", nAnonymizeProfitbaseAmount);
            emit anonymizeProfitbaseAmountChanged(nAnonymizeProfitbaseAmount);
            break;
        case CoinControlFeatures:
