...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::PBC, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (profitbase.conf).");
            QMessageBox::warning(this, "Profitbase Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#f8f6f6; bgcolor:#3a708e;}\n a, span { font-family: monospace; }\n span.addr {color:#3a708e; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #3a708e;}\n td.d0 {font-weight: bold; color:#f8f6f6;}\n h2, h3 { white-space:nowrap; color:#3a708e;}\n a { color:#88f6f6; text-decoration:none; }\n a.nav {color:#3a708e;}\n";
