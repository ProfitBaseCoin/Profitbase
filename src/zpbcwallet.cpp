...
CzPBCWallet::CzPBCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpbc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPBCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPBCSeed_deprecated()) {
                    LogPrintf("%s: Updated zPBC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpbc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpbc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPBCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPBCCount(nCountLastUsed);
    else if (!walletdb.ReadZPBCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPBCWallet::Lock()
{
    seedMaster = 0;
}

void CzPBCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPBCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPBC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPBCWallet::LoadMintPoolFromDB()
{
...
void CzPBCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPBCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPBCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpbcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPBCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPBC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpbcTracker which also adds to database
    pwalletMain->zpbcTracker->Add(dMint, true);
...
void CzPBCWallet::SeedToZPBC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPBCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPBCWallet::GenerateDeterministicZPBC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPBCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPBCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
