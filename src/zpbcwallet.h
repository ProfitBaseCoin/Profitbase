...
#ifndef Profitbase_ZPBCWALLET_H
#define Profitbase_ZPBCWALLET_H
...
public:
    CzPBCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZPBC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZPBC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Profitbase_ZPBCWALLET_H
